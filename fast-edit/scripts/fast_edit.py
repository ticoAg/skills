...
COMMANDS (all support fast-* prefix, e.g. fast-write, fast-paste):
  show FILE START END
    ...
    Example: python3 fast_edit.py show myfile.py 10 20
    Replace line range with new content
    Example: python3 fast_edit.py replace myfile.py 5 7 "new content\\n"
    Insert content after line (LINE=0 for prepend)
    Example: python3 fast_edit.py insert myfile.py 10 "import os\\n"
    Delete line range
    Example: python3 fast_edit.py delete myfile.py 15 20
    Batch edit from JSON (multiple edits in one call)
    Example: python3 fast_edit.py fast-batch --stdin <<< '{"file":"a.py","edits":[...]}'
  paste FILE [--stdin] [--extract] [--base64]
    Save content from clipboard/stdin to file
    Example: echo "code" | python3 fast_edit.py fast-paste output.py --stdin
  write [--stdin] [SPEC]
    Batch write multiple files from JSON
    Example: python3 fast_edit.py fast-write --stdin <<< '{"files":[{"file":"a.py","content":"..."}]}'
  check FILE [--checker NAME]
    Type check Python file (auto-detect: basedpyright/pyright/mypy)
    Example: python3 fast_edit.py check myfile.py
    Save pasted content from OpenCode storage (for large pastes)
    Example: python3 fast_edit.py save-pasted /tmp/file.py
  help
