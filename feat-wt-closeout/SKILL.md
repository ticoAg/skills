...
description: 完成一个基于 git worktree 的 feature：补齐 .feat/{YYYYMMDD-HHMM}-{slug}/ 交付说明并确认需求 vFinal，把 feat/{slug} 用 squash merge 合回原 base 分支，并按 git-commit skill 流程生成最终单一提交，最后清理 worktree（兼容旧版 .cache/codex/features/{slug}/ 结构）。
---
...
注意：脚本不会自动 commit（为了强制走 `git-commit` skill 的 message 生成流程）。
...
### 4. 用 `git-commit` skill 生成最终提交（必须）

调用 `git-commit` skill，为 base 分支当前 staged changes 生成 conventional commit message 并执行提交。

- 重点约束：只基于当前 staged diff 生成 1 个最终 commit
- 不要拆成多个 commit，也不要 amend 既有提交
