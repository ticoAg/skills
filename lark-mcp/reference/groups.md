...
工具: mcp__lark__im_v1_chat_create
data:
  name: "新项目组"  # 可以修改为您需要的群组名称
  owner_id: "ou_xxxxx"  # ⭐ 需要替换为实际的群主 open_id（重要）
  user_id_list: ["ou_xxxxx", "ou_yyyyy"]  # 需要替换为实际的成员 open_id 列表
params:
  user_id_type: "open_id"
useUAT: true  # 使用用户身份
...
data:
  receive_id: "oc_xxxxx"  # 需要替换为步骤2返回的实际 chat_id 
  msg_type: "post"
  content: '{
    "post": {
      "zh_cn": {
...
```yaml
# 创建群组后添加文档权限
工具: mcp__lark__drive_v1_permissionMember_create
data:
